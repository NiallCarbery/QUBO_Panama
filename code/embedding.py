...
def evaluate_mapping_constrained(
    new_coords,
    Q,
    ...
    lambda_origin=1,
):  # penalty factor for being outside allowed radius
    """
    ...
    penalty_neighbors = lambda_neighbors * np.sum(
        np.maximum(0, d_min_allowed - distances) ** 2
    )
...
def run_embedding_for_qubits(n, rep, embeddings_dir, save_embedding=False):
    ...
    if save_embedding:
        ...
        with open(reg_filename, "w") as f:
            json.dump(
                {q: coord.tolist() for q, coord in qubit_positions.items()}, f, indent=4
            )
...
def run_experiment():
    ...
    for n in num_qubits_list:
        ...
        for rep in range(reps):
            save_flag = rep == 0
            t, _ = run_embedding_for_qubits(
                n, rep, embeddings_dir, save_embedding=save_flag
            )
            times.append(t)
        runtime_results[n] = times
        median_t = np.median(times)
        print(
            f"[RESULT] {n} qubits: median time = {median_t:.4f} seconds over {reps} runs"
        )
...
def plot_runtime_results(runtime_results, embeddings_dir):
    ...
    graph_data = {
        ...
        "fit_params": {"A": A, "B": B, "A_error": A_err, "B_error": B_err},
    }
