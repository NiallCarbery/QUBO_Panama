...
lambda_conflict = (
    50  # penalty for mixing long ships (or long with any) in a NeoPanamax slot.
)
lambda_length_np = (
    100  # penalty if two short ships in NeoPanamax violate combined capacity.
)

lambda_once_reward = 65  # Used in evaluate solution, reward fro assigning ship
penalty_infeasible = (
    200  # Used in evalueate solution, Heavy penalty for infeasible assignments
)
